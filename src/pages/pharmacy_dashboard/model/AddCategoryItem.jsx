  ...
  return (
    ...
    <div className='w-[40%] h-[90%] overflow-y-scroll overflow-x-hidden relative  rounded-md bg-[#F0F3F9]'>
        ...
        <div className='flex justify-end w-[95%] mt-4  mx-auto'>
              ...
              }} className="flex items-center justify-center w-full text-white bg-primary hover:bg-primary/50 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2"><span className='inline-block bg-white mr-2 rounded-full text-black'><FiPlus size={18} /></span> Add Drug
                    </button>  
            </div>       
        </div>        
        </div>
            ...
            <div className=" ">
            {
              data.map(({name,description, price}, index) => {
                ...
                return <div key={index} className=" mt-5 min-w-[50%] border border-[#DDDDE8]  px-4 py-2 flex justify-between">
                  <div><GiMedicines size={18} color="#C92B27" /></div>
                  <div>
                    <p > <span className="text-[#52575C]">Name: </span> <span>{name}</span></p>
                    <p > <span className="text-[#52575C]">Sizes: </span> <span>{price}</span></p>
                    <p > <span className="text-[#52575C]">Description: </span> <span>{description}</span></p>
                    <p > <span className="text-[#52575C]">Manufacturer: </span> <span>{name}</span></p>
